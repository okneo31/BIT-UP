...
export default function FuturesOpenOrders() {
  const { openOrders, setOpenOrders, removeOrder } = useFuturesStore();
  ...
  const handleCancel = async (orderId: string) => {
    try {
      const res = await fetch('/api/futures/cancel-order', {
        method: 'POST',
            ...
            <tbody>
              {openOrders.map((order) => (
                <tr key={order.id} className="border-b border-border hover:bg-bg-tertiary">
                  <td className="px-3 py-2 text-text-primary">{order.futures_pairs?.symbol || '--'}</td>
                  <td className={`px-3 py-2 ${order.position_side === 'long' ? 'text-green' : 'text-red'}`}>
                    {order.position_side.toUpperCase()}
                  </td>
                  ...
                  <td className="text-right px-3 py-2 text-accent">{order.leverage}x</td>
