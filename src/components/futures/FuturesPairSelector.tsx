...
export default function FuturesPairSelector() {
  const { currentPair, setCurrentPair } = useFuturesStore();
  ...
  const handleSelect = (symbol: string) => {
    ...
    router.push(`/futures/${symbol}`);
    setIsOpen(false);
  };

  const currentConfig = FUTURES_PAIRS.find(p => p.symbol === currentPair);
  ...
  return (
    <div className="relative">
      ...
      >
        {currentConfig?.base || currentPair.replace('USDT', '')}/USDT
        <span className="text-xs px-1.5 py-0.5 bg-accent/20 text-accent rounded font-medium">Perp</span>
        ...
        <>
          ...
          <div className="absolute top-full left-0 mt-1 w-80 bg-bg-secondary border border-border rounded-lg shadow-lg z-20 overflow-hidden">
            <div className="p-2 border-b border-border">
              <p className="text-xs text-text-secondary px-2">USDT-M Futures</p>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className="text-base font-medium text-text-primary">{pair.base}USDT</span>
                      <span className="text-xs text-text-third">Perp</span>
                      <span className="text-xs text-text-third">{pair.maxLeverage}x</span>
                    </div>
