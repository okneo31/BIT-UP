...
export default function FuturesOrderBook() {
  const { currentPair, orderBook, setOrderBook, currentPrice } = useFuturesStore();
  ...
  const fetchOrderBook = useCallback(async () => {
    ...
    const { data } = await supabase.rpc('get_futures_orderbook', {
    ...
    <div className="bg-bg-secondary rounded-lg border border-border h-full">
      ...
      <div className="text-xs">
        ...
        <div className="px-2 py-1.5 border-y border-border bg-bg-primary">
          ...
          <span className="text-xs text-text-third ml-2">Mark Price</span>
        </div>
